    ...
    <div>
      ...
      <form onSubmit={handleSubmit(submit)} style={{ alignItems: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          <Stack spacing={2} direction="column" className="w-auto   space-y-5  p-5 md:max-w-lg mx-auto">
            <Textfield
              label="Diameter of indentation [d1]"
              variant="outlined"
              register={register}
            ...
            />
            <Textfield
              label="Diameter of indentation [d2]"
              variant="outlined"
              register={register}
              pattern={{ value: /^(0|[1-9]\d*)(\.\d+)?$/, message: 'enter the numbers only' }}
              required={{ value: true, message: 'this field is required' }}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-evenly' }}>
              <Button variant="contained" style={{ width: '80%' }} type="submit">
                Submit
              </Button>
              <button onClick={handleprint} style={{ width: '20%' }} className=" bg-pink-600">
                Print
              </button>
            </div>
            <div>{Object.keys(errors).length !== 0 && <ErrorMessage>please fill all the fields</ErrorMessage>}</div>
            <div style={{ fontSize: '20px' }}>
              Brineel's Hardness Number: {res} kg/mm<sup>2</sup>
            </div>
          </Stack>
        </div>
      </form>
      <div style={{ display: 'none' }}>
        <div ref={ComponentRef}>
          <Print2
            head1="Diameter of indentation [d1]"
            head2="Diameter of indentation [d2]"
            head3="Brineels Hardness Number:"
            first={first}
            second={second}
            res={res}
          />
