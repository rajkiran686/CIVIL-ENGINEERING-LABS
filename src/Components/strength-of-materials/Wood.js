  ...
  var data = [
    {
      ...
      er3: errors?.w3,
      er4: errors?.w4,
      ht1: errors.w1?.message,
      ht2: errors.w2?.message,
      ht3: errors.w3?.message,
      ht4: errors.w4?.message,
      register: register,
...

    {
      ...
      er3: errors1?.w7,
      er4: errors1?.w8,
      ht1: errors1.w5?.message,
      ht2: errors1.w6?.message,
      ht3: errors1.w7?.message,
      ht4: errors1.w8?.message,
      res: res1,
      register: register1,
      handleSubmit: handleSubmit1,
      errors: errors1,
            ...
            <div>
              ...
              <form onSubmit={val.handleSubmit(submit)}>
                <Stack spacing={2} direction="column" className="w-auto   space-y-5  p-5 md:max-w-lg mx-auto">
                  ...
                  />
                  <Textfield
                    label="Enter the width of specimen[B in cm]"
                    variant="outlined"
                    name={val.field2}
                    register={val.register}
                    required={{ value: true, message: 'this field is required' }}
                  ...
                  />
                  <Textfield
                    label="Enter the Depth of specimen[D in cm]"
                    variant="outlined"
                    name={val.field3}
                    register={val.register}
                    required={{ value: true, message: 'this field is required' }}
                  ...
                  />
                  <Textfield
                    label="Enter the Load at specimen fain[p in kN]"
                    variant="outlined"
                    name={val.field4}
                    register={val.register}
                    required={{ value: true, message: 'this field is required' }}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-evenly' }}>
                    ...
                    <button style={{ width: '20%' }} onClick={val.handleprint} className=" bg-pink-600">
                      print
                    </button>
                  </div>
                  {Object.keys(val.errors).length !== 0 && <ErrorMessage>please fill all the fields</ErrorMessage>}
                  <div style={{ fontSize: '20px' }}>
                    Crushing strength,when the loading is parallel to grains:{val.res} kN/cm<sup>2</sup>
                  </div>
                </Stack>
              </form>
              <div style={{ display: 'none' }}>
                <div ref={val.ComponentRef}>
                  <Print4
                    title={val.title}
                    head1="Enter the Length of specimen[L in cm]"
                    head2="Enter the width of specimen[B in cm]"
                    head3="Enter the Depth of specimen[D in cm]"
                    head4="Enter the Load at specimen fain[p in kN]"
                    head5="Crushing strenght:"
                    one={one}
                    two={two}
                    three={three}
                    four={four}
