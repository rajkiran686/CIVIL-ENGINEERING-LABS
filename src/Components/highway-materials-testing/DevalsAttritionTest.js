...
//    var [second,setsecond]=useState();
   ...
   const back=()=>{
    ...
    //   setfirst(data.w1)
    //   setsecond(data.w2)
      res=((data.w1-data.w2)*100/(data.w1)).toFixed(2)
      setres(res)
    }
return (
    <div>
  ...
  HOME
    ...
    <p className='flex justify-center md:text-4xl text-gray-500  pt-10'>DEVAL'S ATTRITION TEST</p>
    <div style={{textAlign:'center'}}>
          <form onSubmit={handleSubmit(submit)}>
    <Stack spacing={2} direction='column' className='w-auto   space-y-5  p-5 md:max-w-lg mx-auto'>
    <Textfield label="Original weight of sample[w1]" variant="outlined" name="w1" register={register} valueAsNumber={true} required={{value:true,message:"this field is required"}}   pattern={{value:/^(0|[1-9]\d*)(\.\d+)?$/,message:'enter the numbers only'}} helperText={errors.w1?.message} error={errors?.w1}/>
    <Textfield label="Weight of material retained on  1.7mm sieve[w2]" variant="outlined" name="w2" register={register} valueAsNumber={true}  required={{value:true,message:"this field is required"}}   pattern={{value:/^(0|[1-9]\d*)(\.\d+)?$/,message:'enter the numbers only'}} helperText={errors.w2?.message} error={errors?.w2}/>
    ...
    <div style={{fontSize:'20px'}}>Deval's attrition value for given sample of aggregate :{res} %</div>
    </Stack>
    </form>
    </div>
...
export default DevalsAttritionTest;